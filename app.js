...
function safeImageSrc(im){
 const src=im&&typeof im.data==='string'&&im.data?im.data:(im&&typeof im.url==='string'?im.url:'');
 return /^(data:image\/|blob:|https?:\/\/)/i.test(src)?src:'';
}
function designImageHTML(im,alt='Gambar desain'){
 const src=safeImageSrc(im);
...
async function optimizeDesignImage(file){
 ...
 const {img}=loaded;const MAX_DIM=1200,TARGET=180*1024,HARD_MAX=300*1024;
 ...
 for(let round=0;round<5;round++){
  ...
  for(const quality of [0.76,0.70,0.64,0.58,0.52,0.46]){
 ...
 *{box-sizing:border-box}body{margin:0;background:#eef2f7}.preview-toolbar{position:sticky;top:0;z-index:9999;display:flex;align-items:center;justify-content:center;gap:10px;padding:12px;background:#111827;box-shadow:0 4px 18px rgba(0,0,0,.2)}.preview-toolbar button{border:0;border-radius:9px;padding:10px 16px;background:#fff;color:#111827;font-weight:800;cursor:pointer}.preview-toolbar .pdf-button{background:#dc2626;color:#fff}.preview-toolbar #pdfStatus{color:#fff;font-size:12px}.print-area{background:#fff;margin:18px auto}.pdf-export .preview-toolbar{display:none!important}@media print{body{background:#fff}.preview-toolbar{display:none!important}.print-area{margin:0 auto!important}}
 ...
 async function saveSamePreviewPDF(){
   ...
   // v3.0.15: selalu salin area preview yang terlihat ke PDF.
   // Jangan gunakan printToPDF Electron karena ukuran halaman print dapat membelah nota.
...
function logActivity(action,detail=''){
 ...
 db.activity=db.activity.slice(0,500); try{save()}catch(err){console.warn('Aktivitas tidak tersimpan:',err)}
}
...
window.permanentDelete=id=>{const o=db.trash.find(x=>x.id===id);if(!o)return;if(!confirm(`Hapus permanen nota ${o.invoice}?\n\nData tidak dapat dipulihkan setelah dihapus.`))return;try{downloadJSON(o,`backup-sebelum-hapus-${o.invoice}.json`)}catch{}window.ShiroganeCloud?.queueImageDeletes?.(o.images||[]);db.trash=db.trash.filter(x=>x.id!==id);logActivity('Hapus Permanen',`${o.invoice} • backup otomatis diunduh`);renderAll();toast('Data dihapus permanen.');};
window.emptyTrash=()=>{if(!db.trash.length)return;const n=db.trash.length;if(!confirm(`Kosongkan Sampah?\n\n${n} data akan dihapus permanen dan tidak dapat dipulihkan.`))return;try{downloadJSON(db.trash,`backup-sampah-${today()}.json`)}catch{}const images=db.trash.flatMap(o=>o.images||[]);window.ShiroganeCloud?.queueImageDeletes?.(images);db.trash=[];logActivity('Kosongkan Sampah',`${n} data • backup otomatis diunduh`);renderAll();toast('Sampah berhasil dikosongkan.');};
...
window.resetSelectedData=()=>{const keys=[];if($('#rOrders')?.checked)keys.push('orders');if($('#rTrash')?.checked)keys.push('trash');if($('#rProducts')?.checked)keys.push('products');if($('#rActivity')?.checked)keys.push('activity');if(!keys.length)return toast('Pilih data yang akan direset.');if(!confirm(`Reset data terpilih?\n\n${keys.join(', ')}\n\nBackup akan diunduh terlebih dahulu.`))return;try{downloadJSON(db,`backup-sebelum-reset-${today()}.json`)}catch{}if(keys.includes('orders')){window.ShiroganeCloud?.queueImageDeletes?.(db.orders.flatMap(o=>o.images||[]));db.orders=[]}if(keys.includes('trash')){window.ShiroganeCloud?.queueImageDeletes?.(db.trash.flatMap(o=>o.images||[]));db.trash=[]}if(keys.includes('products'))db.products=structuredClone(defaults.products);if(keys.includes('activity'))db.activity=[];logActivity('Reset Data',keys.join(', '));renderAll();toast('Data terpilih telah direset.');};
...
/* Hydrate complete database from IndexedDB before cloud synchronization. */
window.SHIROGANE_STORAGE_READY=(async()=>{
 try{
  if(!window.ShiroganeStorage)return db;
  await window.ShiroganeStorage.migrateLegacy(db);
  const full=await window.ShiroganeStorage.loadDB();
  if(full&&typeof full==='object'){
   db=window.ShiroganeStorage.mergeMissingImages(db,full);normalizeSettings();
   try{renderAll()}catch(err){console.error('Gagal merender data IndexedDB:',err)}
  }
  return db;
